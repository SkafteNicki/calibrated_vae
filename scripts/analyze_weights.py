...
from scr.utils import rsetattr, rgetattr
from scr.layers import EnsampleLayer
from scr.classification_models import DeepEnsembles, MixBlockEnsembles, MixLayerEnsembles
...
def get_all_combinations(n_ensemble_size, n_ensemble_layers):
    ...
    for i in range(n_ensemble_layers-1):
        ...
        if i!=0:
            all_combinations = [(*element[0], element[1]) for element in all_combinations]
...
if isinstance(model, list):
    ...
    for i in range(n):
        for j in range(n):
            weight1 = np.concatenate([w.detach().flatten().numpy() for w in model[i].parameters()])
            weight2 = np.concatenate([w.detach().flatten().numpy() for w in model[j].parameters()])
            sim[i, j] = cosine_sim(weight1, weight2)

# mix ensemble
else:
    n_ensemble_layers = len([m for m in model.modules() if isinstance(m, EnsampleLayer)])
    n_ensemble_size = len([m for m in model.modules() if isinstance(m, EnsampleLayer)][0])
    ...
    for i, comb1 in enumerate(combinations):
        for j, comb2 in enumerate(combinations):
            ...
            weight1 = np.concatenate([w.detach().flatten().numpy() for w in temp1.parameters()])
            weight2 = np.concatenate([w.detach().flatten().numpy() for w in temp2.parameters()])
            sim[i,j] = cosine_sim(weight1, weight2)
