...
class EVAE(VAE):
    ...
    def training_epoch_end(self, outputs):
        if self.hparams.ensemble_only_decoder:
            ...
            latents = latents.repeat(self.hparams.n_ensemble, 1).reshape(self.hparams.n_ensemble, *latents.shape)
        else:
            labels = torch.cat([o["labels"] for o in outputs], dim=0)
        ...
        x_var = D.Bernoulli(probs=x_out).entropy().sum(dim=[2,3,4])  # [N_ensample, n_points*n_points]

        for n in range(self.hparams.n_ensemble):
            # plot latent encodings
            for i in labels.unique():
                plt.scatter(
                    latents[n, i==labels, 0].cpu(), latents[n, i==labels, 1].cpu(), 
